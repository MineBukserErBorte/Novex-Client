...
export default function ContentSource({ kind, children }: { kind: ContentKind; children: ReactNode }) {
    ...
    return <section className="content-source-page page">
        <div className="content-source-control"><span className="toolbar-label">Content source</span><NovexSelect label="Content source" value={source} options={[{ value:'modrinth', label:'Modrinth' },{ value:'curseforge', label:'CurseForge — setup required' }]} onChange={value => { const next = value === 'curseforge' ? 'curseforge' : 'modrinth'; localStorage.setItem(key, next); setSource(next); }} /><span className="provider-badge">{source === 'modrinth' ? 'Modrinth' : 'CurseForge'}</span></div>
