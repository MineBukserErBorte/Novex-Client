...
type Dialogs = { confirm: (message: string, title?: string, danger?: boolean) => Promise<boolean>; notice: (message: string) => Promise<boolean> };
...
export default function DialogProvider({ children }: { children: ReactNode }) {
    ...
    return <Context.Provider value={{ confirm: (message, title = 'Confirm action', danger = true) => ask({ title, message, confirm: 'Confirm', danger }), notice: message => ask({ title: 'Novex', message }) }}>
        {children}
